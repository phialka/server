...
async def create_chat(chat_data: schemas.Chat.Create, authorize: JWTAuth = Depends()):
    authorize.jwt_required()
...
async def edit_message(message_id: str, message: schemas.Message.Create):
    return {'status':'OK'}


@chats_router.get('/{chat_id}')
async def get_chat(chat_id:int, authorize: JWTAuth = Depends()):
    authorize.jwt_required()
    chat = await ChatController(authorize.get_jwt_subject()).get_chat(chat_id)
    return chat.view


@chats_router.patch('/{chat_id}')
async def edit_chat(chat_id:int, chat_data: schemas.Chat.Create):
    return {'status':'OK'}
...
async def get_chat_members(chat_id:int, count: Optional[int]=100, offset: Optional[int]=0, authorize: JWTAuth = Depends()):
    authorize.jwt_required()
    return await ChatController(authorize.get_jwt_subject()).get_chat_members(chat_id, count, offset)


@chats_router.post('/{chat_id}/members', response_model=bool)
async def add_chat_members(chat_id:int, user_ids: List[int], authorize: JWTAuth = Depends()):
    authorize.jwt_required()
    return await ChatController(authorize.get_jwt_subject()).add_chat_members(chat_id, user_ids)
