...
async def get_profile_info(authorize: JWTAuth = Depends()):
    ...
    user = await ServerUser(authorize.get_jwt_subject()).create_view()
    return user.view
...
async def edit_profile_info(info: User.EditInfo, authorize: JWTAuth = Depends()):
    ...
    new_user = await (await ServerUser(user_id).edit_info(info)).create_view()
    return new_user.view


@profile_router.patch("/set-online")
async def mark_a_visit(authorize: JWTAuth = Depends()):
    authorize.jwt_required()
    try:
        user_id = authorize.get_jwt_subject()
        await ServerUser(user_id).mark_a_visit()
        return 1
    except:
        return 0
...
async def edit_privacy(options: User.EditSettings, authorize: JWTAuth = Depends()):
    authorize.jwt_required()
    user = await ServerUser(authorize.get_jwt_subject()).edit_privacy_settings(options)
    await user.create_privacy_options_view()
    return user.privacy_options
